...
namespace CallingBotSample
{
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSingleton<OrleansSubscriberService>();

            services.AddSingleton(sp =>
            {
                var botFrameworkAuthentication = sp.GetRequiredService<BotFrameworkAuthentication>();
                var adapter = new CloudAdapter(botFrameworkAuthentication);
                return adapter;
            });

            // Retrieve botAppId from configuration
            string? botAppId = Configuration["MicrosoftAppId"];
            services.AddSingleton(botAppId);
            services.ExtendOptions();
            services.ExtendServices();
            services.RegisterSemanticKernelNativeFunctions();
        }
